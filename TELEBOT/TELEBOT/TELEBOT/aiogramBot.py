...
async def cmd_mute(message: types.Message, command: CommandObject,bot:Bot):
    ...
    vremya_muta = datetime.datetime.now()+datetime.timedelta(hours=kak_dolgo)
    await message.bot.restrict_chat_member(chat_id=message.chat.id, user_id=usrID,permissions=ChatPermissions(can_send_messages=False),until_date=vremya_muta)
    await message.reply(f'{adminNAME} замутировал {usrNAME} на {kak_dolgo} часа')
# ping pong 
@dp.message()
